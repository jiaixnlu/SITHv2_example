...
% 修改pEs为new_pEs,wet修改为fwet
function [wa, zgw, Tr, Es, uex] = sw_balance(IWS, pEc, new_pEs, Ta, Topt, s_VOD, ...
    ...
    if zgw <= 0
        [wa, zgw, Tr, Es, uex] = swb_case0(wa, IWS, pEc, new_pEs, s_tem, s_VOD, ...
            soilpar, pftpar, fwet, zm, zgw);

        % Case 1: groundwater table in layer 1
    elseif zgw > 0 && zgw <= zm(1)
        [wa, zgw, Tr, Es, uex] = swb_case1(wa, IWS, pEc, new_pEs, s_tem, s_VOD, ...
            soilpar, pftpar, fwet, zm, zgw);

        % Case 2: groundwater table in layer 2
    elseif zgw > zm(1) && zgw <= zm(1) + zm(2)
        [wa, zgw, Tr, Es, uex] = swb_case2(wa, IWS, pEc, new_pEs, s_tem, s_VOD, ...
            soilpar, pftpar, fwet, zm, zgw);

        % Case 3: groundwater table in layer 3
    elseif zgw > zm(1) + zm(2) && zgw < zm(1) + zm(2) + zm(3)
        [wa, zgw, Tr, Es, uex] = swb_case3(wa, IWS, pEc, new_pEs, s_tem, s_VOD, ...
            soilpar, pftpar, fwet, zm, zgw);

        % Case 4: groundwater table below layer 3
    else
        [wa, zgw, Tr, Es, uex] = swb_case4(wa, IWS, pEc, new_pEs, s_tem, s_VOD, ...
            soilpar, pftpar, fwet, zm, zgw);
...
% Case 0 -- groundwater overflow %
% ------------------------------ %
function [wa, zgw, Tr, Es, uex] = swb_case0(wa, IWS, pEc, new_pEs, s_tem, s_VOD, ...
    soilpar, pftpar, fwet, zm, zgw)
    % function input:
    % ----------
    ...
    Es = new_pEs; % only consider about the first layer
...
% Case 1 -- groundwater table in layer 1  %
% --------------------------------------- %
function [wa, zgw, Tr, Es, uex] = swb_case1(wa, IWS, pEc, new_pEs, s_tem, s_VOD, ...
    soilpar, pftpar, fwet, zm, zgw)
    % function input:
    % ----------
    % wa      -- soil water content, 3 layers
    ...
    * theta_sat);
    ...
    % actual soil evaporation
    Es = f_sm_s1 .* new_pEs; % only consider about the first layer
...
% Case 2 -- groundwater table in layer 2  %
% --------------------------------------- %
function [wa, zgw, Tr, Es, uex] = swb_case2(wa, IWS, pEc, new_pEs, s_tem, s_VOD, ...
    soilpar, pftpar, fwet, zm, zgw)
    % function input:
    % ----------
    % wa      -- soil water content, 3 layers
    ...
    % actual soil evaporation
    Es = f_sm_s1 .* new_pEs; % only considering the first layer
...
% Case 3 -- groundwater table in layer 3  %
% --------------------------------------- %
function [wa, zgw, Tr, Es, uex] = swb_case3(wa, IWS, pEc, new_pEs, s_tem, s_VOD, ...
    soilpar, pftpar, fwet, zm, zgw)
    % function input:
    % ----------
    % wa      -- soil water content, 3 layers
...
% --------------------------------------- %
function [wa, zgw, Tr, Es, uex] = swb_case4(wa, IWS, pEc, new_pEs, s_tem, s_VOD, ...
    soilpar, pftpar, fwet, zm, zgw)
    % function input:
    % ----------
    % wa      -- soil water content, 3 layers
