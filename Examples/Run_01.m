...
for yr = 1982 : 2020
    ...
    ppm = ParforProgressbar(r_m, 'showWorkerProgress', false); %进度条
    parfor i = 1 : r_m
        
        % read each row, (latitude)
        Rnix = permute(EMO_Rn(i, :, :), [3, 2, 1]); %重新排列
        ...
        for j = 1 : r_n
            ...
            xo = day(EMO_LAItime,"dayofyear");%一年的天数
            xi = 1:1:days;
            LAIii = interp1(xo', LAIi, xi', 'pchip', 'extrap');%10天插值为每日
            LAIii(LAIii < 0) = 0;
