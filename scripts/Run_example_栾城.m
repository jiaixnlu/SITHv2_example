...
for yr = 1989:2016;
    ...
    uptval = load('output_data_spin.mat');
    uptval = uptval.X_upti;
    ...
    X_vals = zeros(days, length(1988:2016), 10); % 仅处理一个列
    % X_upti = zeros(1, length(1988:2016), 5); % 仅处理一个列

    % wa = reshape(waa(i, j, :), [1, 3]); % 三个层深
    % zgw = zgww(i, j);
    % snp = snpp(i, j);
    ...
    year_index = yr - 1988 + 1; % 因为1988是第一年，索引从1开始
    num_days = length(xi); % 获取该年的天数

    % 假设 X_vals 已经包含当前年份的数据，长度为 num_days
    % 将每个变量保存到 x_val 中，确保索引不超出范围
    x_val(1:num_days, year_index, 1) = ETi(1:num_days); % ET
    x_val(1:num_days, year_index, 2) = Tri(1:num_days); % Tr
    x_val(1:num_days, year_index, 3) = Esi(1:num_days); % Es
    x_val(1:num_days, year_index, 4) = Eii(1:num_days); % Ei
    x_val(1:num_days, year_index, 5) = Esbi(1:num_days); % Esb
    x_val(1:num_days, year_index, 6) = SMi(1:num_days, 1); % SM1
    x_val(1:num_days, year_index, 7) = SMi(1:num_days, 2); % SM2
    x_val(1:num_days, year_index, 8) = SMi(1:num_days, 3); % SM3
    x_val(1:num_days, year_index, 9) = RFi(1:num_days); % RF
    x_val(1:num_days, year_index, 10) = GWi(1:num_days); % GW
    ...
    save('upt_vals_01.mat', 'X_upti');
end

save('output_data_Modify.mat', 'x_val', 'X_upti');
